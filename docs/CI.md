...
differs, so use the September formatter for the final style gate. The checked-in
workflow is a reproducible plan, **not proof that hosted CI has run**. Hosted run
and native execution evidence must be recorded after authorized publication.
